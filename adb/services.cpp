...
void restart_root_service(int fd, void *cookie) {
    if (getuid() == 0) {
        ...
        char xosp_release[PROPERTY_VALUE_MAX];
        char xosp_revision[PROPERTY_VALUE_MAX];
        
        property_get("persist.sys.root_access", value, "0");
        property_get("ro.build.type", build_type, "");
        property_get("ro.xosp.release", xosp_release, "");
        property_get("ro.xosp.revision", xosp_revision, "");

        if (strlen(xosp_release) > 0 && strlen(xosp_revision) > 0 && strcmp(build_type, "eng") != 0 && (atoi(value) & 2) != 2) {
